...
- Current canonical thesis HEAD: `b04a55a55ffc75b641ae63f626c1e150f7125a4e` (weekly direction narrowed to chassis model and control validation)
...
- Current evidence status: API comparison and protocol are complete; TCLab simulation smoke test passes with repeatable hashes; Linux BOPTEST locked validation/holdout passes for one FMU on two new dates against fixed PI; exploratory action-conditioned temperature prediction replay of 200 sampled locked-trace states gives MAE 1.6073°C and RMSE 1.6568°C with 0% within 0.5°C, so E8 intervention remains `NOT_EVALUATED` pending a stronger predictor and more testcase/seed action-outcome evidence; toy, simulation, replay, and BOPTEST artifacts remain non-causal evidence.
