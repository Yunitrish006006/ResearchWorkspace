...
for(const q of ["compare public benchmark hybrid residual and intervention evidence across thesis artifacts","causal control intervention public benchmark method verification synchronization"]){
 ...
 assert.ok(p.assignments.some(x=>x.role==="independent-reviewer"),"parallel plans must retain independent review");
 assert.ok(p.assignments.every(x=>Object.hasOwn(x,"model")&&Object.hasOwn(x,"reasoningEffort")));
}
