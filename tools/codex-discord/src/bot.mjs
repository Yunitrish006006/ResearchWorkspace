...
function cliItemDetail(item, completed) {
  ...
  if (type === "collabagenttoolcall" || type === "collabtoolcall") {
    const model = safeProgressText(item?.model) || "模型未回報";
    ...
    try {
      if (action === "status") {
        if (workspaceSync.handlesWorkspace(workspaceName)) {
          try {
            ...
            await interaction.reply({ content: `Shared ResearchWorkspace Codex queue: ${state}.`, ephemeral: true, allowedMentions: { parse: [] } });
          } catch (error) {
            await interaction.reply({ content: `Could not query the shared ResearchWorkspace queue: ${error.message}`, ephemeral: true, allowedMentions: { parse: [] } });
          }
          return;
        }
        const saved = sessions.get(savedKey);
          ...
          try {
            ...
            await interaction.reply({
              content: result.status === "cancelling"
                ? "Sent a cancellation request to the shared ResearchWorkspace Codex task."
                : "The shared ResearchWorkspace Codex queue is already idle.",
              ephemeral: true,
              allowedMentions: { parse: [] }
            });
          } catch (error) {
            await interaction.reply({ content: `Could not cancel the shared ResearchWorkspace task: ${error.message}`, ephemeral: true, allowedMentions: { parse: [] } });
          }
          return;
        }
      ...
      if (workspaceSync.handlesWorkspace(workspaceName)) {
        if (interaction.options.getAttachment("image", false)) {
          await interaction.reply({
            content: "The shared ResearchWorkspace prompt channel currently accepts text only.",
            ephemeral: true,
            allowedMentions: { parse: [] }
          });
          return;
        ...
        await status.edit({
          content: relay.execution === "codex"
            ? "Prompt sent to the shared ResearchWorkspace Codex queue. Progress is mirrored below."
            : "Prompt was recorded by ResearchWorkspace; check the mirrored status for its execution state.",
          allowedMentions: { parse: [] }
        });
        return;
      }
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (workspaceSync.handlesWorkspace(workspaceName)) {
      ...
      try {
        ...
        await message.reply({
          content: relay.execution === "codex"
            ? "已送到共用 ResearchWorkspace Codex 佇列；處理進度會同步到這裡與網頁。"
            : "Prompt 已由 ResearchWorkspace 記錄；請查看同步狀態。",
          allowedMentions: { parse: [] }
        });
      } catch (error) {
        await message.reply({
          content: `無法送到 ResearchWorkspace：${error.message}`,
          allowedMentions: { parse: [] }
        });
      }
      return;
