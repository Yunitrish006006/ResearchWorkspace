...
function workspaceSync(env, allowedChannelIds) {
  const urlValue = env.RESEARCH_WORKSPACE_SYNC_URL?.trim();
  const token = env.RESEARCH_WORKSPACE_SYNC_TOKEN?.trim();
  const channelId = env.RESEARCH_WORKSPACE_SYNC_CHANNEL_ID?.trim();
  const workspaceName = env.RESEARCH_WORKSPACE_SYNC_WORKSPACE?.trim() || "workspace";
  if (!urlValue && !token && !channelId && !env.RESEARCH_WORKSPACE_SYNC_WORKSPACE?.trim()) return null;
  if (!urlValue || !token || !channelId) {
    throw new Error("RESEARCH_WORKSPACE_SYNC_URL, RESEARCH_WORKSPACE_SYNC_TOKEN, and RESEARCH_WORKSPACE_SYNC_CHANNEL_ID must be configured together");
  }
  if (!DISCORD_ID.test(channelId) || !allowedChannelIds.has(channelId)) {
    throw new Error("RESEARCH_WORKSPACE_SYNC_CHANNEL_ID must be an allowed Discord channel ID");
  }
  if (!WORKSPACE_NAME.test(workspaceName)) throw new Error("RESEARCH_WORKSPACE_SYNC_WORKSPACE must be a configured workspace name");
  let url;
  try {
    url = new URL(urlValue);
  } catch {
    throw new Error("RESEARCH_WORKSPACE_SYNC_URL must be a valid URL");
  }
  const loopback = url.protocol === "http:" && ["127.0.0.1", "localhost", "[::1]", "::1"].includes(url.hostname);
  if (!loopback) throw new Error("RESEARCH_WORKSPACE_SYNC_URL must use an http loopback host");
  if (token.length < 16) throw new Error("RESEARCH_WORKSPACE_SYNC_TOKEN must contain at least 16 characters");
...
export function loadConfig(env = process.env) {
  ...
  if (sync && !workspaces.has(sync.workspaceName)) {
    throw new Error("RESEARCH_WORKSPACE_SYNC_WORKSPACE must name a configured workspace");
  }
  return Object.freeze({
