...
test("configuration requires explicit user, channel and workspace allowlists", () => {
  ...
  const sync = loadConfig({
    ...
    RESEARCH_WORKSPACE_SYNC_TOKEN: "test-private-sync-token",
    RESEARCH_WORKSPACE_SYNC_CHANNEL_ID: "423456789012345678"
  }).workspaceSync;
  ...
  assert.throws(() => loadConfig({ ...env, RESEARCH_WORKSPACE_SYNC_URL: "https://example.com" }), /configured together/);
  assert.throws(() => loadConfig({
    ...env,
    RESEARCH_WORKSPACE_SYNC_URL: "http://example.com",
    RESEARCH_WORKSPACE_SYNC_TOKEN: "test-private-sync-token",
    RESEARCH_WORKSPACE_SYNC_CHANNEL_ID: "423456789012345678"
  }), /loopback host/);
});
