...
class FloatingPanel extends StatelessWidget {
    ...
    final selected = await showModalBottomSheet<FloatingPanelDock>(
      ...
      builder: (context) => SafeArea(
        child: ListView(
          shrinkWrap: true,
          children: [
            ListTile(title: Text('移動「$title」'), subtitle: const Text('選擇固定位置；面板會保持在畫面內。')),
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, constraints) {
      ...
      final panelHeight = expandedHeight == null ? null : math.min(expandedHeight!, availableBodyHeight).toDouble();
      return Align(
                    ...
                    child: SizedBox(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(child: Text(title, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800))),
                          IconButton(tooltip: '選擇面板位置', onPressed: () => _chooseDock(context), icon: const Icon(Icons.open_with, size: 18)),
                          IconButton(tooltip: collapsed ? '展開面板' : '收合面板', onPressed: () => onCollapsedChanged(!collapsed), icon: Icon(collapsed ? Icons.expand_more : Icons.expand_less, size: 20)),
                          if (onClose != null) IconButton(tooltip: '關閉面板', onPressed: onClose, icon: const Icon(Icons.close, size: 18)),
